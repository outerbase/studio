...
import { deleteLocalBaseDialog } from "./(outerbase)/local/dialog-base-delete";
import { createLocalBoardDialog } from "./(outerbase)/local/dialog-board-create";
import { deleteLocalBoardDialog } from "./(outerbase)/local/dialog-board-delete";
import {
  useLocalConnectionList,
  useLocalDashboardList,
} from "./(outerbase)/local/hooks";
import {
  ResourceItemList,
  ResourceItemProps,
} from "./(outerbase)/resource-item-helper";
        ...
        <DropdownMenu modal={false}>
          ...
          >
            ...
            <div className="flex gap-4">
              <Link
                to="/playground/client?template=northwind"
              ...
              <Link
                to="/playground/client?template=chinook"
                className="bg-secondary hover:bg-primary hover:text-primary-foreground flex cursor-pointer flex-col gap-2 rounded p-2 text-left text-base"
              >
                <span className="font-bold">Chinook</span>
                <span className="text-sm">
                  The Chinook Database is a sample digital media store database
